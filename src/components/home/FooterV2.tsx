...
export function FooterV2() {
  return (
    <footer className="bg-brand-ink text-gray-300 rounded-t-[40px]">
      <div className="container-custom py-16 lg:py-20">
        <div className="grid lg:grid-cols-12 gap-10">
          {/* Brand */}
          <div className="lg:col-span-4">
            <Link href="/" className="flex items-center">
              <span className="text-2xl font-bold text-white tracking-tight">clicsHQ</span>
            </Link>
            ...
            <div className="mt-6 flex gap-2">
              {SOCIAL_ICONS.map((s) => (
                ...
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">{s.svg}</svg>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-3 gap-8">
            {COLUMNS.map((col) => (
              <div key={col.heading}>
                <h3 className="text-base font-semibold text-white">{col.heading}</h3>
