...
type Props = {
  ...
  primary?: { label?: string | null; url?: string | null } | null
  secondary?: { label?: string | null; url?: string | null } | null
}

export function FinalCTA({
  heading = "Bring your team's work into one connected place.",
  description = 'Plan projects, manage tasks, collaborate with your team, and automate the busywork with AI-powered workflows.',
  primary = { label: 'Start for free', url: '#' },
  secondary = { label: 'Get a demo', url: '#' },
}: Props = {}) {
  return (
    <section className="relative isolate bg-gradient-to-b from-gray-100 via-gray-200 to-gray-300 overflow-hidden">
      ...
      <div className="container-custom relative py-24 sm:py-32 max-w-5xl">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-brand-ink leading-tight max-w-2xl whitespace-pre-line">
          {heading}
        </h2>
        {description && (
          <p className="mt-4 text-base text-gray-600 max-w-xl">{description}</p>
        )}
