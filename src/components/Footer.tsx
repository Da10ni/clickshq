...
type FooterData = {
  ...
  columns?: Array<{ heading: string; links?: Array<{ label: string; url: string; newTab?: boolean | null }> | null }> | null
  socialLinks?: Array<{ platform: string; url: string }> | null
}

export function FooterComponent({ data }: { data: FooterData }) {
  return (
    <footer className="bg-gray-950 text-gray-400">
      <div className="container-custom pt-16 pb-10 lg:pt-20">
        <div className="grid grid-cols-2 md:grid-cols-12 gap-10">
          ...
          <div className="col-span-2 md:col-span-4">
            <Link href="/" className="flex items-center gap-2.5">
              ...
              <span className="text-lg font-bold tracking-tight text-white">ClicksHQ</span>
            </Link>
            <p className="mt-4 text-sm leading-relaxed max-w-xs">
              The all-in-one project management platform built for modern teams. Plan, track, and deliver — together.
            </p>
              ...
              <div className="mt-6 flex gap-2">
                {data.socialLinks.map((s, i) => (
                  <a
                    ...
                    className="grid h-9 w-9 place-items-center rounded-lg bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white transition-colors"
                  >
            ...
            <div key={i} className="md:col-span-2">
              ...
              <ul className="mt-4 space-y-3">
                {col.links?.map((link, j) => (
                  <li key={j}>
                    <Link href={link.url} target={link.newTab ? '_blank' : undefined} className="text-sm text-gray-400 hover:text-white transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-14 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-gray-500">
            {data.copyright || `© ${new Date().getFullYear()} ClicksHQ. All rights reserved.`}
          </p>
